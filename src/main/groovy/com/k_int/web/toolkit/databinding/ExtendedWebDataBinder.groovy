...
class ExtendedWebDataBinder extends GrailsWebDataBinder {
	...
	private static <T> T[] getAllBeansByTypeFromContext( final Class<T> type, final ApplicationContext context) {
		ApplicationContext ctx = context
		List<T> matches = ctx.getBeansOfType(type, true, true).values().collect()
		while (ctx.parent) {
			ctx = ctx.parent
			matches += ctx.getBeansOfType(type, true, true).values()
		}
		
		matches.toArray(new T[0])
	}
	...
	ExtendedWebDataBinder(GrailsApplication grailsApplication) {
		...
		this.setTrimStrings(configurationProperties.isTrimStrings());
		this.setAutoGrowCollectionLimit(configurationProperties.getAutoGrowCollectionLimit());
		...
		final FormattedValueConverter[] formattedValueConverters = getAllBeansByTypeFromContext(FormattedValueConverter.class, mainContext)
		this.setFormattedValueConverters(formattedValueConverters);

		final TypedStructuredBindingEditor<?>[] structuredBindingEditors = getAllBeansByTypeFromContext(TypedStructuredBindingEditor.class, mainContext)
		this.setStructuredBindingEditors(structuredBindingEditors);

		final DataBindingListener[] dataBindingListeners = getAllBeansByTypeFromContext(DataBindingListener.class, mainContext)
		this.setDataBindingListeners(dataBindingListeners);

		this.setMessageSource(mainContext.getBean("messageSource", MessageSource.class));
	}
